...
public class Day4Solver
{
    ...
    public void SolvePartOne()
    {
        ...
        foreach (var line in _text)
        {
            ...
            var charStringsRaw = new List<string>(line.Split('-'));
    ...
    public void getShiftedStrings(List<string> unshiftedStrings)
    {
        foreach (var unshiftedString in unshiftedStrings)
        {
            ...
            foreach (var character in unshiftedString)
            {
                ...
                if (shiftedCharAsInt > 122) shiftedCharAsInt = 96 + shiftedCharAsInt % 122;
                shiftedString += (char)shiftedCharAsInt;
                if (shiftedString.Equals("north")) targetRoom = true;
            }
        }
    }
    ...
    {
        var sortedRegistry = _letterRegistry.OrderByDescending(x => x.Value).ThenBy(k => k.Key).Select(k => k.Key)
            .Take(5);

        for (var index = 0; index < judgmentCharacters.Length; index++)
        {
            var letter = judgmentCharacters[index];
            if (letter != sortedRegistry.ElementAt(index)) return false;
        }
